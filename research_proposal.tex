...
To study consciousness in AI, in particular LLMs based AI we cannot only rely on input-output behavioural experiments to assess the probability that current and future LLMs are conscious. The main reason is that LLMs are precisely the sort of systems that have been trained to simulate human behaviour, in particular humans engaged in tasks relying on natural language.

Instead it is preferable to assess consciousness in AI systems by monitoring both behaviour of some LLM and the dynamics of its neural activations in LLMs while they engage in that behaviour. This is because we have a state of the art knowledge of measures of consciousness in humans and we can use this knowledge to assess the consciousness of AI systems.

Let $x_{t+1} = M(w, x_t)$ be the output of some AI model $M$ with parameters $w$ (for example $M$ could be a transformer architecture). We consider $X_t = e(x_t)$ to be the embedding of the token $x_t$. The collection of vector $X^(l)_t$ for each layers of the architecture are the neural activations of the model $M$. They are related to each others deterministically via the parameters $w$. Since the tokens $x_t$ are random variables sampled from the distribution of human language, the activations $X_t$ are also random variables with some underlying probability distribution. The neural computation is the collection of activations at all time steps $(X_t)_{t\in E}$ for some epoch of time $E$. 
We would like to have some measure $C$ such that $C(X_{t\in E})$ measure the degree (or classify) of consciousness over the epoch $E$.
\\
\\
For example, suppose that we instruct the model $M$ with a sequence of tokens $q$. After seeing the sequence $q$ the model take a sequence of actions in the form of a sequence of tokens $b$: the behavior of the model $M$. We are interested in measuring $C((X_t)|q)$ for $t$ running over the indices of the tokens in the sequence $b$ given instruction $q$. In particular, we want to compare $C((X_t)|q_1)$ and $C((X_t)|q_2)$ for two different instructions $q_1$ and $q_2$ instructing the model to engage in behaviour mimicking different states of consciousness in humans. We want to test the hypothesis that $C_M(X_{t\in E}|q_1) > C_M(X_{t\in E}|q_2)$ when we know that $C_H(X_{t\in E}|q_1) > C_H(X_{t\in E}|q_2)$ where $H$ is a human performing a similar task.
\\
\\
%============================================================================
...
% Talk about the different complexity measures of consciousness
% Lempel ziv complexity
% Perturbation complexity index 
% Correlation with global states of consciousness in humans.
% Doubt about wether complexity is a good measure of consciousness and risk that it is too human-centric.
% Talk about causal density and weak IIT measurees

% Paragraph on measures of integration and differentiation in the brain. (could perhaps use some of my thesis here)

% Paragraph on how they are correlated with global states of consciousness in humans.
...
% Explain measurable proxies for consciousness-relevant features
...
% Take example of models that we could use
% Take example of experiments that we could run
